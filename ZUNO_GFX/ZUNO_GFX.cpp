...
uint8_t ZUNO_GFX::printChar(const uint8_t *sim_buf)
{

	uint8_t		w_sim = *sim_buf;
...
// uint8_t printChar(uint32_t unicode)
// {
...
// uint32_t printChar(const char *str, size_t len)
// {
// 	uint32_t unic;
...
//проеряем 
//отправляем в принт чар(указатель на символ)
...
size_t ZUNO_GFX::write(const uint8_t *str, size_t len)
{
	uint32_t	unic;
	size_t		ret = 0;
	char *sub_str = (char*)str;
